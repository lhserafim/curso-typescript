...
const falarCom = (nome: string) => console.log(`Falar com ${nome}`)
falarCom('Luiz')
...
function contagemRegressiva(inicio: number = 3): void { // passando param padrão
    console.log(inicio)
    while(inicio > 0) {
        inicio -- 
        console.log(inicio)
...
// Promises
// Fazendo sem promises, uso de callback

// Usando callback
function esperar3s() {
    setTimeout(() => {
        console.log('3s depois...')
    }, 3000) // em milisegundos
}

esperar3s()

// Usando promises
// vantagem: encadear chamadas
function esperar3sPromise() {
    return new Promise((resolve: any) => {
        setTimeout(() => {
            console.log('3s depois promise...')
        }, 3000) // em milisegundos
    })
}
esperar3sPromise().then(x => console.log(x))
